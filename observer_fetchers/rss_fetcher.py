...
def fetch_rss_headlines(url, verbose=False):
    try:
        if verbose: print(f"    [FETCH] Trying to pull RSS feed from: {url}")
        response = requests.get(url)
        response.raise_for_status()

        if verbose: print("    [FETCH] RSS feed pulled successfully.")
        ...
        if verbose: print(f"    [FETCH] Found {len(headlines)} headlines.")
        return headlines
